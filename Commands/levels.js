...
module.exports.run = async (bot, message, args, coreMessage, prefix) => {
    ...
    let entries = modules.createLevelsBoard(levels, userIDs, xp, amount)

    if (entries.join("\n\n").length > 2048) return message.channel.send(`That number is too high. I can't display that many entries.`)
    let embed = new Discord.MessageEmbed()
        .setColor(`5d10e9`)
        .setTitle(`Leaderboard - Krystal Levels`)
        .setDescription(entries.join("\n\n"))
    if (!args[1]) embed.setFooter(`You can see more entries by specifying an amount of users to show with o!levels (user count).`)
